...
// v27.9.5b — Help article detail (hunter view). Audience filter
// excludes guide-only articles. Static-generated at build via
// generateStaticParams over the hunter-visible article set.

export async function generateStaticParams() {
  return articlesForAudience('hunter').map((a) => ({ slug: a.slug }))
}

export const dynamicParams = false

export default async function HunterHelpArticlePage({
  params,
}: {
  ...
  const article = getArticleBySlug(slug, 'hunter')
  if (!article) notFound()
  const category = getCategory(article.category)
  const related = (article.related ?? [])
    .map((s) => getArticleBySlug(s, 'hunter'))
    .filter((a): a is NonNullable<typeof a> => a !== null)

  return (
    <main className="bb-app-main">
      <Link href="/app/h/support" className="bb-text-action" style={{ marginBottom: '0.85rem', display: 'inline-flex', alignItems: 'center', gap: '0.4rem' }}>
      ...
      {related.length > 0 && (
        <section className="bb-help-section">
          ...
          <ul className="bb-help-grid">
            {related.map((r) => (
              <li key={r.slug}>
                <Link href={`/app/h/support/${r.slug}`} className="bb-help-card">
