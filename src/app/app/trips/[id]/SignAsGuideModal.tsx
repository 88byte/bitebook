...
import SignaturePad, { type SignaturePadHandle } from './log/SignaturePad'
import { signWaiverAsGuideAction } from '../../_lib/waiver-sign'

// v27.2.0.3 — guide-side variant of SignWaiverModal. Posts to
// signWaiverAsGuideAction and anchors on tripDocId instead of an
// action id.

export default function SignAsGuideModal({
  open,
  onClose,
  tripDocId,
  docLabel,
  ...
  function clearPad() { findPad()?.clear() }

  function save() {
    setError(null)
    const handle = findPad()
    if (!handle) { setError('Signature pad isn’t ready yet — try once more.'); return }
    if (handle.isEmpty()) { setError('Draw your signature first.'); return }
    const dataUrl = handle.toDataURL()
    if (!dataUrl) { setError('Couldn’t serialize the signature.'); return }
    startTransition(async () => {
      const res = await signWaiverAsGuideAction(tripDocId, dataUrl)
      if ('error' in res) {
        setError(res.error)
        return
      }
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.5rem' }}>
          <h2 className="bb-form-section-head" style={{ margin: 0, display: 'inline-flex', alignItems: 'center', gap: '0.4rem' }}>
          ...
          <button
            ...
            style={{ border: 'none', background: 'transparent', color: 'var(--color-ink-soft)', cursor: 'pointer', padding: '0.25rem' }}
          >
        ...
        <p className="bb-form-help" style={{ margin: 0 }}>
          Drawing your signature here paints the guide signature onto{' '}
          <strong>{docLabel}</strong> for this trip and stamps any
          “Date Signed” fields with today’s date.
        </p>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
          <button
            type="button" onClick={clearPad} disabled={pending}
            className="bb-text-action bb-text-action-copper"
            style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.85rem', background: 'transparent', border: 'none', padding: 0, cursor: 'pointer' }}
          >
            <RotateCcw size={14} aria-hidden="true" />
            Clear
          </button>
          <span style={{ flex: 1 }} />
          <button type="button" onClick={onClose} disabled={pending} className="bb-btn-secondary">
            Cancel
          </button>
          <button
            type="button" onClick={save} disabled={pending || isEmpty}
            className="bb-cta-sm"
