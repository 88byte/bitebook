...
import GuideInviteSignupForm from './GuideInviteSignupForm'

// v28.1.0e.0 Sprint 3.3 — Free signup path for network-guide invitees.
// Guide-network seats are covered by the outfitter's subscription, so
// invitees never hit Stripe. Mirrors /accept-admin-invite/signup.

type SP = Promise<{ token?: string }>

export default async function GuideInviteSignupPage({ searchParams }: { searchParams: SP }) {
  ...
  return (
    <main className="bb-app-main">
      <div className="bb-form-narrow" style={{ marginTop: '1rem' }}>
        <div className="mb-3">
          <Link
            href={`/accept-guide-network-invite?token=${encodeURIComponent(token)}`}
        ...
        <header>
          ...
          <h1 className="bb-page-title" style={{ color: '#FAF7F2' }}>Create your free guide account</h1>
          <p className="bb-page-sub" style={{ color: '#FAF7F2', opacity: 0.85 }}>
            Joining <strong>{orgName}</strong> as a guide. Your seat is covered by the outfitter, so there is nothing to pay.
          </p>
        </header>

        <section className="bb-tile bb-form-section" style={{ marginTop: '1rem' }}>
          <div className="bb-tile-body">
            <GuideInviteSignupForm
              token={token}
