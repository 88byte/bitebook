import Link from 'next/link'
import { redirect } from 'next/navigation'
import { createAdminClient } from '@/lib/supabase/admin'
import { createClient } from '@/lib/supabase/server'
import AcceptGuideInviteButton from './AcceptGuideInviteButton'

// v28.1.0e.0 Sprint 3.3 — Accept page for outfitter guide-network invites.
// Mirrors /accept-admin-invite. Server-renders invite metadata and
// branches on auth state:
//   valid + signed in     → "Accept invite" CTA
//   valid + signed out    → sign in or create account options
//   invalid / expired     → clean error view

type SP = Promise<{ token?: string }>

export default async function AcceptGuideNetworkInvitePage({ searchParams }: { searchParams: SP }) {
  const sp = await searchParams
  const token = (sp.token || '').trim()

  if (!token) return <InvalidView reason="missing_token" />

  const admin = createAdminClient()
  const { data: invite } = await admin
    .from('outfitter_guide_network_invites')
    .select('id, org_id, invited_email, status, expires_at, invited_by_profile_id')
    .eq('token', token)
    .maybeSingle()
  if (!invite) return <InvalidView reason="not_found" />
  if (invite.status === 'revoked') return <InvalidView reason="revoked" />
  if (invite.status === 'accepted') return <InvalidView reason="accepted" />
  if (new Date(invite.expires_at).getTime() < Date.now()) {
    return <InvalidView reason="expired" />
  }

  const { data: org } = await admin
    .from('outfitter_orgs')
    .select('id, name, owner_profile_id')
    .eq('id', invite.org_id)
    .maybeSingle()
  if (!org) return <InvalidView reason="not_found" />

  let inviterName = 'The outfitter'
  try {
    const { data: prof } = await admin
      .from('profiles')
      .select('display_name')
      .eq('id', invite.invited_by_profile_id)
      .maybeSingle()
    if (prof?.display_name) inviterName = prof.display_name
  } catch {/* ignore */}

  // Are they signed in?
  const sb = await createClient()
  const { data: { user } } = await sb.auth.getUser()

  // Already an active member of this network? Skip to dashboard.
  if (user) {
    const { data: existing } = await admin
      .from('outfitter_guide_network')
      .select('id, status')
      .eq('org_id', org.id)
      .eq('guide_profile_id', user.id)
      .maybeSingle()
    if (existing?.status === 'active') redirect('/app')
  }

  return (
    <main className="bb-app-main">
      <div className="bb-form-narrow" style={{ marginTop: '1rem' }}>
        <header>
          <p className="bb-page-eyebrow" style={{ color: 'var(--color-copper)' }}>Guide network invite</p>
          <h1 className="bb-page-title" style={{ color: '#FAF7F2' }}>
            {inviterName} invited you to guide for {org.name}
          </h1>
          <p className="bb-page-sub" style={{ color: '#FAF7F2', opacity: 0.85 }}>
            When you accept, you can run trips, log harvests, and share documents under {org.name}. Your seat is covered by their subscription.
          </p>
        </header>

        <section className="bb-tile bb-form-section" style={{ marginTop: '1rem' }}>
          <div className="bb-tile-body">
            <p className="bb-form-help" style={{ marginTop: 0 }}>
              Invited email: <strong>{invite.invited_email}</strong>
            </p>

            <p className="bb-form-help" style={{ margin: '0.5rem 0 0' }}>
              Joining {org.name}&rsquo;s guide network requires an active Bite Book Guide subscription ($9/mo, 7-day free trial). The outfitter covers admin tooling, not your guide seat.
            </p>

            {!user ? (
              <>
                <p style={{ margin: '0.75rem 0' }}>
                  Sign in if you already have a Bite Book Guide account, or start a free trial below.
                </p>
                <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginTop: '0.75rem' }}>
                  <Link
                    href={`/login?next=${encodeURIComponent(`/accept-guide-network-invite?token=${token}`)}`}
                    className="bb-cta-sm"
                  >
                    Sign in
                  </Link>
                  <Link
                    href={`/signup?network_invite_token=${encodeURIComponent(token)}`}
                    className="bb-btn-secondary"
                  >
                    Start free trial
                  </Link>
                </div>
              </>
            ) : (
              <>
                <p style={{ margin: '0.75rem 0' }}>
                  Signed in as <strong>{user.email}</strong>. Click below to accept.
                </p>
                <AcceptGuideInviteButton token={token} />
              </>
            )}
          </div>
        </section>
      </div>
    </main>
  )
}

function InvalidView({ reason }: { reason: 'missing_token' | 'not_found' | 'revoked' | 'accepted' | 'expired' }) {
  const copy = (() => {
    switch (reason) {
      case 'missing_token': return 'No invite token was provided.'
      case 'not_found': return 'This invite is invalid or no longer exists.'
      case 'revoked': return 'This invite has been revoked.'
      case 'accepted': return 'This invite has already been accepted.'
      case 'expired': return 'This invite has expired. Ask the outfitter to send a new one.'
    }
  })()
  return (
    <main className="bb-app-main">
      <div className="bb-form-narrow" style={{ marginTop: '1rem' }}>
        <header>
          <p className="bb-page-eyebrow" style={{ color: 'var(--color-copper)' }}>Guide network invite</p>
          <h1 className="bb-page-title" style={{ color: '#FAF7F2' }}>Invite unavailable</h1>
        </header>
        <section className="bb-tile bb-form-section" style={{ marginTop: '1rem' }}>
          <div className="bb-tile-body">
            <p>{copy}</p>
            <p style={{ marginTop: '0.75rem' }}>
              <Link href="/login" className="bb-text-action bb-text-action-copper">Back to sign in</Link>
            </p>
          </div>
        </section>
      </div>
    </main>
  )
}
